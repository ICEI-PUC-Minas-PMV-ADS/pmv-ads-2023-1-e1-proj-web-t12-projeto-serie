...
export default function Search({
  ...
  async function loadMore(){  
    ...
    if (searchType === 'all') {
      const res1 = await fetch(`https://api.themoviedb.org/3/search/tv?query=${searchName}&include_adult=false&language=pt-BR&page=${page}`, {
        headers: {
          'Authorization': `Bearer ${process.env.NEXT_PUBLIC_TMDB_API_KEY}`,
          'accept': 'application/json',
        },
      ...
      const res2 = await fetch(`https://api.themoviedb.org/3/search/movie?query=${searchName}&include_adult=false&language=pt-BR&page=${page}`, {
        headers: {
          'Authorization': `Bearer ${process.env.NEXT_PUBLIC_TMDB_API_KEY}`,
          'accept': 'application/json',
        },
  ...
  return (
    ...
    <div className="pt-8">
      ...
      <h2 className='font-bold pb-4 pl-8'> Pesquisa por &quot;{searchName}&quot; em Séries:</h2>
       : null}
      {searchType === "all" ? 
      <h2 className='font-bold pb-4 pl-8'> Pesquisa por &quot;{searchName}&quot; em Séries e Filmes:</h2>
       : null}
        {/* Cards */}
       {searchType === "all" ?
         ...
         {resultAll.map((movie: movieType, index) => (
           <Link
           className=""
           href={`/${(Math.floor((index % 40) / 20) === 0) ? 'series' : 'movie'}/${movie.id}`}
           key={index}
         >
        ...
        <div className='grid grid-cols-2 sm:grid-cols-5 lg:grid-cols-8 gap-4 items-center p-12'>
          {result.results.map((movie: movieType, index) => (
            <Link className='' href={`/${index <= 10 ? "movie" : "serie"}/${movie.id}`} key={index}>
              <div className="flex flex-col">
                <img
                  className=''
                  src={`https://image.tmdb.org/t/p/w500/${movie.poster_path}`} alt="" />
...
export const getServerSideProps: GetServerSideProps<SearchResultsProps> = async (context) => {
  ...
  if (searchType === 'all') {
    const res1 = await fetch(`https://api.themoviedb.org/3/search/tv?query=${searchName}&include_adult=false&language=pt-BR&page=1`, {
      headers: {
        'Authorization': `Bearer ${process.env.TMDB_API_KEY}`,
        'accept': 'application/json',
      },
    ...
    const res2 = await fetch(`https://api.themoviedb.org/3/search/movie?query=${searchName}&include_adult=false&language=pt-BR&page=1`, {
      headers: {
        'Authorization': `Bearer ${process.env.TMDB_API_KEY}`,
        'accept': 'application/json',
      },
